...
async def dial(phone_number: str, room_name: str, name: str | None = None) -> None:
    """Create the room and dispatch the outbound agent into it."""
    lk = api.LiveKitAPI()
    try:
        await lk.room.create_room(api.CreateRoomRequest(name=room_name))

        # The agent reads this metadata to know who to call and lookup profile.
        await lk.agent_dispatch.create_dispatch(
            api.CreateAgentDispatchRequest(
                agent_name=AGENT_NAME,
                room=room_name,
                metadata=json.dumps({"phone_number": phone_number, "name": name}),
            )
        )
    finally:
        await lk.aclose()
...
def main() -> None:
    ...
    parser.add_argument(
        "--to",
        required=True,
        help="Number to call, in E.164 format (e.g. +15551234567) or SIP URI",
    )
    parser.add_argument(
        "--name",
        default=None,
        help="Name of the user to look up in the database",
    )
    parser.add_argument(
        "--room",
        default=None,
    ...
    asyncio.run(dial(args.to, room_name, args.name))
